...
//        grid2.setGridLinesVisible(true);
    ...
    private VBox addVBoxim(){
        im1 = new ImageView(new Image(Gui.class.
                            getResourceAsStream("/resources/pm.png")));
...
//                System.out.println(fileName);        
    ...
    private void setProfiles(){
        // initial position chart
                try {
                chartinil = new calc.Chart().createinv(wheell, raill);
                chartinir = new calc.Chart().create(wheelr, railr);
                } catch (FileNotFoundException ex) {
                    read_diag = new Alert(AlertType.ERROR);                    
                    read_diag.setTitle("ERROR");
                    read_diag.setHeaderText(ex.toString());
    ...
    private void finalCharts(){
        try {
             // plot results
             chart00 = new calc.Chart().createinv(wlt, rlt, pl);
             chart0 = new calc.Chart().create(wrt, rrt, pr);
             chart1 = new eqcon.Chart().create2(out[0],out[1],out[0][id3mm],out[1][id3mm]);
             chart2 = new eqcon.Chart().create0(dl, drf);
             chart3 = new eqcon.Chart().create1(dl, dar);
